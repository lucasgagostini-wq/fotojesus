...
-- Funnel Analytics: order_id nullable — eventos de topo (landing/upload) ocorrem
-- antes de existir um pedido. Nenhuma lógica de negócio depende desta tabela e o
-- insert tolera falha (api/track sempre responde 204).
-- FK ON DELETE CASCADE (2026-06-17): apagar um pedido em `orders` remove
-- automaticamente seus eventos de funil (direto no Supabase ou via /exclude).
...
CREATE INDEX IF NOT EXISTS funnel_events_agg_idx ON funnel_events (route_source, event_name);

-- FK idempotente: limpa órfãos antes (caso pedidos já tenham sido apagados) e
-- adiciona a constraint só se ainda não existir.
DELETE FROM funnel_events fe
WHERE fe.order_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM orders o WHERE o.id = fe.order_id);
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'funnel_events_order_id_fkey') THEN
    ALTER TABLE funnel_events
      ADD CONSTRAINT funnel_events_order_id_fkey
      FOREIGN KEY (order_id) REFERENCES orders(id) ON DELETE CASCADE;
  END IF;
END $$;

-- Agregação eficiente (sessões distintas por rota/evento) feita no Postgres.
CREATE OR REPLACE VIEW funnel_event_counts AS
SELECT route_source, event_name, count(DISTINCT session_id) AS sessions
FROM funnel_events
GROUP BY route_source, event_name;
