  ...
  switch (command) {
    ...
    case "reset-funnel": {
      try {
        ...
        // funnel_events.id é BIGINT GENERATED BY DEFAULT AS IDENTITY — gt(0) apaga
        // todas as linhas (IDs auto-increment começam em 1). NÃO toca em `orders`,
        // então o fundo do funil (PIX/aprovado/entregue) permanece intacto.
        const { error: fErr, count } = await supabase
          .from("funnel_events")
          .delete({ count: "exact" })
          .gt("id", 0);
        if (fErr) throw new Error(`funnel_events: ${fErr.message}`);

        console.log(`[bmth/admin-cmd/reset-funnel] ${count ?? "?"} eventos removidos por ${session.u}`);
        return res.status(200).json({
          ok: true,
          message: `Métricas do funil zeradas: ${count ?? "?"} evento(s) removido(s). PIX/aprovado/entregue derivam de pedidos reais e não foram afetados.`,
        });
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : "Internal error";
        console.error("[bmth/admin-cmd/reset-funnel]", err);
        return res.status(500).json({ error: message });
      }
    }
